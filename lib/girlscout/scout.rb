...
module Girlscout
  class Scout
    ...
    def crawl(host = nil, port = nil, start = 0, limit = 50000, wait = nil)
      ...
      urls.each do |uri|
        begin
          ...
          sleep(wait) if wait
          uri = uri.to_s
          print ["4", "5"].include?(response[0]) ? "F" : "."
    ...
    def print_responses(dir = nil)
      Scout.print_responses(@responses, dir)
    end
    ...
    class << self
      def print_responses(responses, dir = nil)
        ...
        if responses.empty?
          ...
          FileUtils.touch("#{dir}/results.yml")
          results = File.open("#{dir}/results.yml") { |yf| YAML.load(yf) } || Hash.new()
          responses.each do |response, urls|
            results[response] = results.has_key?(response) ? results[response] + urls.size : urls.size
            yml = File.open("#{dir}/#{response}.yml", "a+")
            urls.each do |url|
              yml.puts url
            end
            yml.close
          end
          File.open("#{dir}/results.yml", 'w' ) do |out|
